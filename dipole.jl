...
function image_dipole(p::Params, P1::Position, P2::Position)
  # x-axis dipole
  (q, r) = (p.q, p.r);
  (x1, y1) = (P1.x, P1.y);
  (x2, y2) = (P2.x, P2.y);
  ...
  return (x,y) -> q * (1/sqrt((x-x1)^2 + (y-y1)^2) - 1/sqrt((x-x2)^2 + (y-y2)^2)) - q01 * (1/sqrt((x-a1)^2 + (y-b1)^2)) + q02 * (1/sqrt((x-a2)^2 + (y-b2)^2))
end
